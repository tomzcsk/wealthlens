...
| Phase 6 — Debt & Polish | ✅ Completed | เก็บหนี้เทคนิค (F49) · รื้อหน้าวิเคราะห์ (F50) · PWA — ติดจอโฮม + เปิดได้ไม่มีเน็ต (F51) |
...
13. **Service worker อัปเดตแบบ `prompt` เท่านั้น ไม่ใช่ `autoUpdate` (F51)** — reload เงียบ ๆ ตอนกำลังกรอกฟอร์ม = สิ่งที่พิมพ์ค้างหาย. มี SW ใหม่รอ → ขึ้นแถบ `UpdatePrompt` ให้กดเอง (ไม่ใช้ toast: toast ไม่มีปุ่มกดและหายเองใน 4 วินาที — prompt ที่หายเองคือ prompt ที่พลาด). และ **`/api/gold-price` + Google APIs เป็น `NetworkOnly` เสมอ** — ราคาทองสดถูกแช่ = net worth คิดจากราคาเมื่อวานโดยไม่มีใครรู้; token ค้างใน cache = บั๊ก + ความเสี่ยง. `devOptions.enabled: false` — SW ใน dev ทำให้แก้โค้ดแล้วไม่เห็นผล
